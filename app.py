...
# ---------------------------------------------------------------------
    ...
    for i in range(len(s)):
        ...
        try:
            r = npf.irr(cf)
            irr[i] = r if r is not None else np.nan
        except Exception:
            irr[i] = np.nan
    ...
    baseline_negative_irr = (df["irr_pct"] < 0).sum()
    shocked_negative_irr = (s["irr_pct"] < 0).sum()
    ...
    c2.metric("Properties with Negative IRR", f"{shocked_negative_irr}", delta=f"{shocked_negative_irr - baseline_negative_irr:+d}", delta_color="inverse")
